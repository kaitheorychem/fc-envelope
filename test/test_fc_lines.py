...
def test_single_mode_at_zero_temperature_is_the_poisson_series(single_mode):
    ...
    mode = single_mode[0]
    result = lines_quietly(single_mode, temperature=0.0, min_intensity=1e-12)
    ...
    for line in result.lines:
        ...
        assert line.intensity == pytest.approx(line.fc_factor)


def test_zero_phonon_line_comes_first_and_sits_at_zero(multi_mode, temperature):
    """E = 0 の ZPL は遷移を持たない線として必ず含まれる。"""
    result = lines_quietly(multi_mode, temperature=temperature, min_intensity=1e-8)
...
def test_lines_are_sorted_by_intensity(multi_mode, temperature):
    result = lines_quietly(multi_mode, temperature=temperature, min_intensity=1e-6)
    intensities = result.intensities
    assert np.all(np.diff(intensities) <= 0.0)
    assert np.all(intensities >= result.min_intensity)
...
def test_broadened_lines_reproduce_the_envelope(multi_mode, temperature):
    ...
    result = lines_quietly(
        multi_mode, temperature=temperature, min_intensity=1e-7, max_lines=200000
    )

    assert result.diagnostics.beam_truncated is False
    reconstructed = broadened(result, envelope.energy, sigma)
    peak = float(np.max(envelope.intensity))
    residual = float(np.max(np.abs(reconstructed - envelope.intensity)))
    # 取りこぼした強度（1 - captured）が誤差の上限を与える。
    assert residual / peak < 10.0 * (1.0 - result.diagnostics.captured_intensity) + 1e-9


def test_total_intensity_approaches_one(multi_mode, temperature):
    """全遷移にわたる線強度の総和は厳密に 1。閾値の分だけ取りこぼす。"""
    coarse = lines_quietly(multi_mode, temperature=temperature, min_intensity=1e-4)
    fine = lines_quietly(
        multi_mode, temperature=temperature, min_intensity=1e-7, max_lines=200000
    )
    assert coarse.diagnostics.captured_intensity < fine.diagnostics.captured_intensity
    assert fine.diagnostics.captured_intensity == pytest.approx(1.0, abs=1e-3)
...
def test_threshold_is_exhaustive(multi_mode):
    ...
    coarse = lines_quietly(multi_mode, temperature=300.0, min_intensity=1e-3)
    fine = lines_quietly(
        multi_mode, temperature=300.0, min_intensity=1e-8, max_lines=200000
    )
    ...
    expected = {key(line) for line in fine.lines if line.intensity >= 1e-3}
...
def test_max_lines_truncates_and_warns(multi_mode):
    with pytest.warns(NumericalQualityWarning, match="max_lines"):
        result = compute_fc_lines(
            multi_mode, temperature=300.0, min_intensity=1e-8, max_lines=50
        )
...
def test_low_coverage_warns(multi_mode):
    ...
    with pytest.warns(NumericalQualityWarning, match="captured_intensity"):
        result = compute_fc_lines(multi_mode, temperature=300.0, min_intensity=0.02)
    assert result.diagnostics.n_lines > 0
    assert result.diagnostics.captured_intensity < 0.9
...
def test_no_line_above_the_threshold_reports_the_strongest(multi_mode):
    with pytest.warns(NumericalQualityWarning, match="strongest possible line"):
        result = compute_fc_lines(multi_mode, temperature=300.0, min_intensity=1.0)
    assert result.lines == ()
    assert result.diagnostics.captured_intensity == 0.0
...
def test_recurrence_limited_initial_state_warns():
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        result = compute_fc_lines(modes, temperature=300.0, min_intensity=1e-4)
...
@pytest.mark.parametrize(
    ...
    [
        {"temperature": -1.0},
        {"temperature": 0.0, "min_intensity": 0.0},
        {"temperature": 0.0, "min_intensity": 2.0},
...
def test_arrays_follow_the_line_order(multi_mode):
    result = lines_quietly(multi_mode, temperature=77.0, min_intensity=1e-5)
    assert result.energies.shape == result.intensities.shape == result.fc_factors.shape
