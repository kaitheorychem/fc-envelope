...
ファイルの `schema_version` どうしも別々に上がる。結果ファイルは入力ファイルの形を写さない
ので、入力の書き方が変わっても結果ファイルの版は上がらない（今はどちらも 4、ADR-0080）。
