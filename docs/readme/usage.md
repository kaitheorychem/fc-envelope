...
プログラムの値はそのまま写せる。見出しに `10^-4 a.u.` のような倍率が付いていれば、倍率も
単位の側に写す（[別名と倍率](#別名と倍率)）。
...
[[modes]]
frequency = 500.0
coupling = [-0.3, "10^-4 a.u."]   # 出力の VCC 列と見出しの単位をそのまま
```
...
名前の前に空白で区切って**倍率**を置くと、値全体をその数倍して読む。倍率は小数・指数表記
（`1e-3`）か 10 の整数乗（`10^-3`）で、正の数に限る。
...
[grid]
unit = "10^-3 eV"                 # 窓とグリッド間隔をまとめて meV で
```

`"1e-3eV"` のように空白を省くと、全体が 1 つの名前として読まれて未知の単位で止まる。
実効設定には別名を正式名に置き換えた**正式形**が出る（`"10^-4 a.u."` → V の欄なら
`"10^-4 hartree/(bohr*sqrt(m_e))"`）。倍率は書いた字面のまま残る。
