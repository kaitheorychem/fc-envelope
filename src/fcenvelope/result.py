...
@dataclass(frozen=True, slots=True)
class LinesDiagnostics:
    """離散 FC 因子の数値品質。エンベロープ側の `EnvelopeDiagnostics` とは指標が異なる。"""
...
class LinesResult:
    ...
    selection: Selection
    """入力エコー。どの線を保持したかのつまみを丸ごと持つ（ADR-0035）。"""
